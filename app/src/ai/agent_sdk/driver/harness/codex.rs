...
/// Warp to run without a manual hook review in driver sessions.
...
impl CodexHarnessRunner {
    fn new(
        ...
        let command = codex_command(cli_command, &prompt_path);
...
fn prepare_codex_config_toml(
    ...
    sanitize_codex_mcp_servers(&mut doc);
...
/// Retain only local or process-backed MCP entries already present in Codex's config.
fn sanitize_codex_mcp_servers(doc: &mut toml_edit::DocumentMut) {
    let Some(mcp_tbl) = doc
        .get_mut("mcp_servers")
    ...
    let names_to_remove: Vec<String> = mcp_tbl
        .iter()
        .filter_map(|(name, entry)| {
            let entry = entry.as_table()?;
            let is_local = entry.contains_key("command")
                || entry
                    .get("url")
                    .and_then(toml_edit::Item::as_str)
                    .is_some_and(is_local_codex_mcp_url);
            (!is_local).then_some(name.to_owned())
        })
        .collect();
...
fn is_local_codex_mcp_url(url: &str) -> bool {
    ...
    match url.scheme() {
        "data" => true,
        "file" => is_local_codex_mcp_host(url.host_str()),
...
fn is_local_codex_mcp_host(host: Option<&str>) -> bool {
