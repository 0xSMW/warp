...
impl InitProjectModel {
    ...
    pub fn start(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // Disabled in local-only mode: remote language-server discovery.
        ...
        // Disabled in local-only mode: cloud environment setup.
        // self.set_step(
    ...
    pub fn should_have_available_steps(path: &Path, ctx: &warpui::AppContext) -> bool {
        ...
        let has_pending_project_scoped_rules =
            FILES_TO_CHECK.iter().all(|name| !path.join(name).exists());
