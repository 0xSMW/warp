...
//!
//! The RPC remains available for remote-server IPC, but local-only builds do not
//! gather or upload cloud snapshots.
...
/// Keep the remote-server IPC response path available in local-only builds without uploading a
/// cloud snapshot.
#[cfg(not(test))]
pub(crate) async fn gather_and_upload_handoff_snapshot(
    ...
    log::debug!("Remote handoff snapshot upload is disabled in local-only mode");
    Ok(None)
}
