...
fn assert_cloud_command_rejected(args: &[&str]) {
    let command_name = args[1];
    let mut command = Args::clap_command();
    command.build();

    assert!(
        command.find_subcommand(command_name).is_none(),
        "cloud command {command_name} must not be registered"
    );
    command
        .try_get_matches_from(args)
        .expect_err("cloud command must be rejected by the runtime parser");
    Args::try_parse_from(args).expect_err("cloud command must be rejected by the derived parser");
}

#[test]
fn cloud_environment_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "environment", "list"]);
}

#[test]
fn cloud_run_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "run", "list"]);
}

#[test]
fn cloud_task_alias_is_rejected() {
    assert_cloud_command_rejected(&["warp", "task", "list"]);
}

#[test]
fn cloud_model_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "model", "list"]);
}

#[test]
fn cloud_memory_store_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "memory-store", "list"]);
}

#[test]
fn cloud_memory_stores_alias_is_rejected() {
    assert_cloud_command_rejected(&["warp", "memory-stores", "list"]);
}

#[test]
fn cloud_memory_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "memory", "list", "store-123"]);
}

#[test]
fn cloud_login_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "login"]);
}

#[test]
fn cloud_logout_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "logout"]);
}

#[test]
fn cloud_whoami_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "whoami"]);
}

#[test]
fn cloud_provider_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "provider", "list"]);
}

#[test]
fn cloud_integration_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "integration", "list"]);
}

#[test]
fn cloud_schedule_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "schedule", "list"]);
}

#[test]
fn cloud_secret_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "secret", "list"]);
}

#[test]
fn cloud_federate_command_is_rejected() {
    assert_cloud_command_rejected(&[
        "warp",
        "federate",
        "issue-token",
        "--run-id",
...
#[test]
fn cloud_harness_support_command_is_rejected() {
    assert_cloud_command_rejected(&[
        "warp",
        "harness-support",
        "--run-id",
...
#[test]
fn cloud_artifact_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "artifact", "get", "artifact-123"]);
}

#[test]
fn cloud_api_key_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "api-key", "list"]);
}

#[test]
fn cloud_runner_command_is_rejected() {
    assert_cloud_command_rejected(&["warp", "runner", "list", "--team=team_uid00000000000123"]);
}

#[test]
fn mcp_list_accepts_global_output_format() {
    let args = Args::try_parse_from(["warp", "mcp", "list", "--output-format", "ndjson"]).unwrap();

    assert_eq!(args.output_format(), OutputFormat::Ndjson);
    let Some(Command::CommandLine(command)) = args.command() else {
        panic!("Expected `warp mcp list` command");
    };
    assert!(matches!(
        command.as_ref(),
        CliCommand::MCP(MCPCommand::List)
    ));
}
...
fn help_hides_api_key_env_value() {
    ...
    let mcp_help = command
        .find_subcommand_mut("mcp")
        .expect("mcp subcommand exists")
        .render_long_help()
        .to_string();
    let args = Args::try_parse_from(["warp", "mcp", "list"]).expect("API key env var should parse");

    restore_env_var("WARP_API_KEY", previous_api_key);

    for help in [&top_level_help, &mcp_help] {
        assert!(
...
fn api_key_before_subcommand_parses() {
    ...
    let args = Args::try_parse_from(["warp", "--api-key", "test-key", "mcp", "list"]).unwrap();
...
fn debug_before_subcommand_parses() {
    ...
    let args = Args::try_parse_from(["warp", "--debug", "mcp", "list"]).unwrap();
...
fn multiple_global_flags_before_subcommand_parse() {
    ...
    let args =
        Args::try_parse_from(["warp", "--api-key", "test-key", "--debug", "mcp", "list"]).unwrap();
